...
VIP=$(grep -E '^\s+ip:' "$TALOS_DIR/patches/controlplane/vip.yaml" | awk '{print $2}' | head -1)
CLUSTER_NAME=$(grep -E '^\s+name:' "$TALOS_DIR/patches/all/cluster-name.yaml" | awk '{print $2}' | head -1)
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  echo "  all: $(ls "$TALOS_DIR/patches/all/"*.yaml | xargs -n1 basename | tr '\n' ' ')"
  echo "  controlplane: $(ls "$TALOS_DIR/patches/controlplane/"*.yaml | xargs -n1 basename | tr '\n' ' ')"
  echo "  workers: $(ls "$TALOS_DIR/patches/workers/"*.yaml | xargs -n1 basename | tr '\n' ' ')"
  echo "[dry-run] Nodes:"
...
if [[ ! -f "$SECRETS_SOPS" ]]; then
  echo "Error: $SECRETS_SOPS not found. Run: talosctl gen secrets > talos/secrets.yaml && hack/edit-secret.sh talos/secrets.yaml" >&2
  exit 1
fi
...
for f in "$TALOS_DIR/patches/workers/"*.yaml; do
  WORKER_PATCHES+=(--config-patch-worker "@$f")
done
